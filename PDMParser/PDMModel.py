...
PhysOpts=""".decode('utf-8')
        self.firstphysicaldiag = PDMPhysicalDiagram(u'默认面板', u'firstdiag')
        self.packagelist=[]

    def addpackage(self,name,code,comment=None):
        package=PDMPackage(name,code,comment)
        self.firstphysicaldiag.addsymbol(name,code,'Package',self.getidno(),comment)
    ...
    def toxmlelement(self):
        ...
        shortcut=etree.SubElement(dbms, '{object}Shortcut', Id='o3')
        etree.SubElement(shortcut, '{attribute}ObjectID').text = '799FC735-C3DE-4C11-B26B-033A995F0F11'
        etree.SubElement(shortcut, '{attribute}Name').text = 'ORACLE Version 11g'
        etree.SubElement(shortcut, '{attribute}Code').text = 'ORA11GR1'
        etree.SubElement(shortcut, '{attribute}CreationDate').text = str(time.time())
        etree.SubElement(shortcut, '{attribute}Creator').text = 'ZhuJian'
        etree.SubElement(shortcut, '{attribute}ModificationDate').text = str(time.time())
        etree.SubElement(shortcut, '{attribute}Modifier').text = 'ZhuJian'
        etree.SubElement(shortcut, '{attribute}TargetStereotype')
        etree.SubElement(shortcut, '{attribute}TargetID').text = 'BAE632F3-AC04-4059-9893-259ABA89351C'
        etree.SubElement(shortcut, '{attribute}TargetClassID').text = '4BA9F647-DAB1-11D1-9944-006097355D9B'
        ...
        symbols=etree.SubElement(model,'{collection}Symbols')
        symbols.append(self.firstphysicaldiag.toxmlelement())
        ...
        SessionShortcuts = etree.SubElement(model, '{collection}SessionShortcuts')
    ...
    def output(self):
        ...
        eletree.write(self.name+'.pdm',method='xml',encoding='utf-8',pretty_print=True)
